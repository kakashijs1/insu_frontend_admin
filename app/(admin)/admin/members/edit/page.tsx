...
export default function EditMemberPage() {
  ...
  const members = [
    {
      id: 1,
      name: "คุณสมชาย ใจดีมาก",
      phone: "081-234-5678",
      email: "somchai.j@gmail.com",
      type: "ประกันรถยนต์ชั้น 1",
      status: "Active",
      joined: "12 ธ.ค. 2025",
    },
    {
      id: 2,
      name: "คุณวิภา รักษ์ดี",
      phone: "095-888-9999",
      email: "wipa.r@hotmail.com",
      type: "ประกันสุขภาพ",
      status: "Active",
      joined: "05 ม.ค. 2026",
    },
    {
      id: 3,
      name: "คุณมานะ มุ่งมั่น",
      phone: "082-111-2222",
      email: "mana.m@outlook.com",
      type: "ประกันชีวิต",
      status: "Pending",
      joined: "10 ม.ค. 2026",
    },
    {
      id: 4,
      name: "คุณรุ่งนภา ฟ้าใส",
      phone: "089-777-6666",
      email: "rung.n@gmail.com",
      type: "ประกันรถยนต์ชั้น 2+",
      status: "Active",
      joined: "15 ม.ค. 2026",
    },
  ];
      ...
      <div className="bg-white rounded-[2rem] border border-slate-200 shadow-sm overflow-hidden">
          ...
          <table className="w-full text-left">
            <thead>
              <tr className="bg-slate-50/80 text-slate-500 text-[11px] uppercase tracking-[0.1em] font-bold">
                ...
                <th className="px-6 py-4 border-b border-slate-100 text-right uppercase">
                  จัดการ
                </th>
              </tr>
            </thead>
              ...
              {members.map((member) => (
                ...
                >
                  ...
                  <td className="px-6 py-4 text-center">
                    ...
                    >
                      <div
                        className={`w-1.5 h-1.5 rounded-full ${member.status === "Active" ? "bg-emerald-500" : "bg-orange-500"}`}
                      />
                      {member.status}
                    </span>
                  </td>
                  <td className="px-6 py-4 text-right">
                    <div className="flex justify-end gap-2">
                      <button
                        className="p-2 text-slate-400 hover:text-teal-600 hover:bg-white rounded-xl transition-all border border-transparent hover:border-slate-100 shadow-none hover:shadow-sm"
                        title="แก้ไข"
                      >
                        <Edit2 size={16} />
                      </button>
                      <button
                        className="p-2 text-slate-400 hover:text-danger hover:bg-white rounded-xl transition-all border border-transparent hover:border-danger-light shadow-none hover:shadow-sm"
                        title="ลบ"
                      >
                        <Trash2 size={16} />
                      </button>
        ...
        <div className="p-6 bg-slate-50/50 border-t border-slate-100 flex items-center justify-between text-xs font-medium text-slate-400">
          ...
          <div className="flex gap-2 font-sans">
            <button className="px-4 py-2 bg-white border border-slate-200 rounded-xl hover:bg-slate-50 disabled:opacity-50">
              ก่อนหน้า
            </button>
            <button className="px-4 py-2 bg-white border border-slate-200 rounded-xl hover:bg-slate-50">
              ถัดไป
            </button>
          </div>
        </div>
